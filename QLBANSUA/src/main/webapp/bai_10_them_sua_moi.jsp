...
<%
	...
	if(result>0){
	%>
		<p align="center"><b>Thêm sản phẩm thành công</b></p><br>
	...
	%>
		...
		<tr class="style37">
			...
			<br>
				...
				<b><i>Lợi ích:</i></b><br>
				<%=rs.getString("loi_ich") %><br>
				<p"><b><i>Trọng lượng:</i></b> <span><%=rs.getInt("trong_luong")%></span>  gr - <b><i>Đơn giá:</i></b> <span><%=nf.format(rs.getInt("don_gia")) %> VNĐ</span></span></p>
			</td>
		</tr>
...
<%} %>
	...
	<%}
		...
		e.printStackTrace();
	%>
	<p align="center"><b>Không thể thêm hãy kiểm tra lại thông tin</b></p>
